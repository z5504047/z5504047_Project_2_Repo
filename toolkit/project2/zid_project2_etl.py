...
# ----------------------------------------------------------------------------
def read_prc_csv(tic, start, end, prc_col='Adj Close'):# 读取一段时间内的股票价格
    ...
    # <COMPLETE THIS PART>
    # Convert the stock symbols to lowercase
    tic = tic.lower()
    # Use os.path.join to add the path
    pth = os.path.join(cfg.DATADIR, f'{tic}_prc.csv')
    # Read the stock price data file
    df = pd.read_csv(pth)
    # Convert the data type of the 'Date' column to datetime format
    ...
    # Rename the filtered price data to the stock symbols
    ser = df.rename(tic)
    return ser
...
# ----------------------------------------------------------------------------
def daily_return_cal(prc):# Calculate the ratio of each day's return to the previous day's return minus 1
    ...
    # Ensure the returned series has the same name as the input series
...
# ----------------------------------------------------------------------------
def monthly_return_cal(prc):# Calculate and return a Pandas series of monthly returns for an individual stock
    ...
    """
    # <COMPLETE THIS PART>
    # Resample monthly and calculate the number of trading days in each month
    monthly_counts = prc.resample('M').count()
    # Resample monthly and take the price of the last trading day of each month
    monthly_prc = prc.resample('M').last()
    # Filter out months with fewer than 18 trading days
    valid_monthly_prc = monthly_prc[monthly_counts >= 18]
    # Calculate the monthly returns for valid months
    monthly_returns = valid_monthly_prc.pct_change().dropna()
    # Convert the index to PeriodIndex (Year-Month format) for readability
    monthly_returns.index = monthly_returns.index.to_period('M')
    # Set the index name to 'Year_Month'
    monthly_returns.index.name = 'Year_Month'
    # Ensure the series name is consistent with the input series
...
def aj_ret_dict(tickers, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    # Calculate daily returns and monthly returns using a dictionary during initialization
    ...
    # Create a return dictionary that contains DataFrames of daily returns and monthly returns
    ret_dict = {
