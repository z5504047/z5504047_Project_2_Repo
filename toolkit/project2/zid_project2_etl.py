...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    # Ensure the index is a DatetimeIndex and handle missing values
...
def daily_return_cal(prc):
    ...
    # Ensure the series name remains the same as `prc`
...
def monthly_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    # Resample the price series to get end-of-month prices
    monthly_prices = prc.resample('M').last()

    # Calculate monthly returns
    monthly_returns = monthly_prices.pct_change().dropna()

    # Remove months with fewer than 18 entries in the original daily series
    valid_months = prc.resample('M').size() >= 18
...
def aj_ret_dict(tickers, start, end):
    ...
    for ticker in tickers:
        ...
        daily_returns[ticker.lower()] = daily_return_cal(prc_series)
        monthly_returns[ticker.lower()] = monthly_return_cal(prc_series)

    # Combine all individual ticker returns into a single DataFrame
