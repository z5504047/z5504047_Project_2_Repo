...
from toolkit.project2 import zid_project2_characteristics as cha, zid_project2_etl as etl
from toolkit.project2 import zid_project2_portfolio as pf
...
# ----------------------------------------------------------------------------
def get_avg(df: pd.DataFrame, year: int):
...
def get_cumulative_ret(df: pd.DataFrame):
...
df = DM_Ret_dict['Daily']
avg_returns_2008 = get_avg(df, 2008)
Q1_ANSWER = avg_returns_2008.idxmin()
...
#     Use the output dictionary, DM_Ret_dict, and auxiliary function in this script
#     to do the calculation.
Q2_ANSWER = avg_returns_2008.loc[Q1_ANSWER]
...
#     Use the output dictionary, DM_Ret_dict, and auxiliary function in this script
#     to do the calculation.
df_monthly = DM_Ret_dict['Monthly']
avg_returns_2019 = get_avg(df_monthly, 2019)
Q3_ANSWER = avg_returns_2019.idxmax()
...
#     Use the output dictionary, DM_Ret_dict, and auxiliary function in this script
#     to do the calculation.
Q4_ANSWER = avg_returns_2019.loc[Q3_ANSWER]
...
#     to do the calculation.
volatility_tsla_2010 = Vol_Ret_mrg_df['tsla_vol'].loc['2010']
Q5_ANSWER = volatility_tsla_2010.mean()
...
#     Use the output dataframe, Vol_Ret_mrg_df, and auxiliary function in this script
#     to do the calculation.
volatility_v_2008 = Vol_Ret_mrg_df['v_vol'].loc['2008']
volatility_v_2018 = Vol_Ret_mrg_df['v_vol'].loc['2018']
Q6_ANSWER = (volatility_v_2008.mean() / volatility_v_2018.mean()).round(1)
...
effective_months_tsla_2010 = Vol_Ret_mrg_df[['tsla', 'tsla_vol']].loc['2010'].dropna().shape[0]
Q7_ANSWER = effective_months_tsla_2010
...
#     to do the calculation.
lowest_vol_quantile_2019 = EW_LS_pf_df.loc['2019', 'ls']
Q9_ANSWER = lowest_vol_quantile_2019.mean()
...
# Example process to find the stock with the lowest average daily return
def t_stat(EW_LS_pf_df: pd.DataFrame):
    """
    Calculate statistical measures for the long-short portfolio.

    Parameters:
    EW_LS_pf_df (pd.DataFrame): DataFrame containing portfolio returns with a 'ls' column for long-short returns.

    Returns:
    pd.DataFrame: A DataFrame with one row and three columns: 'ls_bar', 'ls_t', and 'n_obs'.
    """
    ls = EW_LS_pf_df['ls']
    ls_bar = ls.mean()
    n_obs = ls.count()
    ls_t = ls_bar / (ls.std() / (n_obs ** 0.5))

    return pd.DataFrame({'ls_bar': [round(ls_bar, 4)], 'ls_t': [round(ls_t, 4)], 'n_obs': [n_obs]})
