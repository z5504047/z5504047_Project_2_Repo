...
Q7_ANSWER = '5'
# 获取'TSLA'股票的日回报和月回报数据
ret_dict = etl.aj_ret_dict(['TSLA'], '2010-01-01', '2010-12-31')
# 使用vol_cal函数计算TSLA的每月波动性
tsla_vol_df = cha.vol_cal(ret_dict, 'vol', ['Daily'])
...
# print(f"2010年TSLA的有效年月数量为: {effective_months_2010}")
...
# print(f"EW_LS_pf_df 数据框有 {rows} 行和 {cols} 列。")
...
# 计算2019年最低波动性分位的平均等权重投资组合回报
avg_return_lowest_volatility_2019 = ew_ls_2019_df[lowest_volatility_column].mean()
# print(f"2019年最低总波动性分位的平均等权重投资组合回报为: {avg_return_lowest_volatility_2019:.4f}")
