...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    # <COMPLETE THIS PART>
    # Extract daily returns data from the dictionary and make a copy
    ...
    # Calculate the monthly volatility (standard deviation) for each stock
    # If there are fewer than 18 daily returns in a month, set the volatility to NaN
    vol = daily_returns.groupby(daily_returns.index).apply(
        lambda x: x.std() if len(x) >= 18 else pd.Series([np.nan] * len(x.columns), index=x.columns)
    )

    # Add the characteristic suffix to column names
...
def merge_tables(ret, df_cha, cha_name):
    ...
    """
    # <COMPLETE THIS PART>
    # Extract monthly returns data from the 'ret' dictionary
    monthly_returns = ret['Monthly'].copy()

    # Merge the monthly returns with the characteristics dataframe
    merged_df = monthly_returns.merge(df_cha, how='left', left_index=True, right_index=True)

    # Shift the characteristics columns one month forward
    characteristics_cols = [col for col in merged_df.columns if col.endswith(f"_{cha_name}")]
    merged_df[characteristics_cols] = merged_df[characteristics_cols].shift(1)

    # Drop rows where all values are NaN (if any exist)
    merged_df = merged_df.dropna(how='all')

    # Ensure the index name is set to 'Year_Month'
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    # Calculate the characteristics using the vol_cal function
    df_cha = vol_cal(ret, cha_name, ret_freq_use)

    # Merge the characteristics with the monthly returns using the merge_tables function
